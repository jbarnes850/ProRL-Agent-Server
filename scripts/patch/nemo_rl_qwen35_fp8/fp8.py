...
def _get_module_from_param_name(model, name: str):
    # Composite checkpoints use Hugging Face parameter names while vLLM may
    # expose a different internal module hierarchy. Use the model's native
    # mapper for classification only; model.load_weights() applies the same
    # mapper when it consumes the original names below.
    mapper = getattr(model, "hf_to_vllm_mapper", None)
    if mapper is not None:
        mapped_names = mapper.apply_list([name])
        if len(mapped_names) != 1:
            print(
                f"Warning: Could not map parameter '{name}' to exactly one "
                f"vLLM name: {mapped_names}"
            )
            return None
        name = mapped_names[0]
