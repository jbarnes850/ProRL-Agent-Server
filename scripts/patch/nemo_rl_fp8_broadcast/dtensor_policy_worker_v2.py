...
from nemo_rl.utils.fp8_broadcast_quant import (
    FP8_BLOCK_SIZE as _FP8_BLOCK_SIZE,
    cast_tensor_to_fp8_blockwise_producer,
    fp8_broadcast_enabled as _fp8_broadcast_enabled,
    fp8_broadcast_scale_shape as _fp8_broadcast_scale_shape,
    fp8_broadcast_should_quantize as _fp8_broadcast_should_quantize,
)
...
# the producer instead of shipping bf16 and requantizing post-receipt in vLLM.
# The block-quant math lives in the torch-only nemo_rl.utils.fp8_broadcast_quant
# module bind-mounted alongside this file; fed weight.to(bf16).to(float32) it is
# bit-identical to the vLLM post-receipt requant, proved run-agnostically by
# scripts/patch/nemo_rl_fp8_broadcast/test_fp8_equivalence.py. Gated by
# NRL_FP8_BROADCAST=1.
def dtensor_params_generator_fp8_broadcast(
    model: nn.Module, target_dtype: torch.dtype
) -> Generator[tuple[str, torch.Tensor], None, None]:
    """Like dtensor_params_generator, but block-quantizes fp8-eligible weights.

    For a name matching the fp8 predicate, yields (name, fp8_data) followed by
    (name + '_scale_inv', scale). All other tensors pass through as target_dtype
    exactly as the base generator. The interleave order matches prepare_refit_info
    so the packed broadcast stays aligned with the consumer's state_dict_info.
    """
    for name, tensor in dtensor_params_generator(model, target_dtype):
        if _fp8_broadcast_should_quantize(name, tensor):
            fp_data, descale = cast_tensor_to_fp8_blockwise_producer(
                tensor.to(torch.float32), _FP8_BLOCK_SIZE
            )
            descale = torch.squeeze(descale, dim=-1).contiguous()
            yield name, fp_data.contiguous()
...
class DTensorPolicyWorkerV2Impl(
    ...
    def prepare_refit_info(self) -> Optional[dict[str, Any]]:
        ...
        fp8_broadcast = _fp8_broadcast_enabled()
        state_dict_info = {}
        for name, tensor in self.model.state_dict().items():
            ...
            for adapted_fqn, adapted_tensor in adapted_fqn_tensors:
                if fp8_broadcast and _fp8_broadcast_should_quantize(
                    adapted_fqn, adapted_tensor
                ):
                    # fp8 data (1 byte/elem) + block scale, matching the pair the
                    # fp8-aware broadcast generator yields for this name.
                    state_dict_info[adapted_fqn] = (
                        adapted_tensor.shape,
                        torch.float8_e4m3fn,
                    )
                    state_dict_info[adapted_fqn + "_scale_inv"] = (
                        _fp8_broadcast_scale_shape(adapted_tensor.shape),
                        torch.float32,
                    )
                else:
    ...
    def broadcast_weights_for_collective(
        ...
        if _fp8_broadcast_enabled():
            broadcast_iterator = dtensor_params_generator_fp8_broadcast(
                self.model, self.dtype
            )
            print(
                "[fp8_broadcast] producer: fp8 block-quant broadcast ENABLED",
                flush=True,
            )
        else:
            broadcast_iterator = dtensor_params_generator(self.model, self.dtype)

        packed_broadcast_producer(
            iterator=broadcast_iterator,
