...
class VllmInternalWorkerExtension:
    ...
    def _load_weights(self, weights):
        ...
        if fp8.is_fp8_model(self.model_runner.vllm_config):
            if os.getenv("NRL_FP8_BROADCAST") == "1":
                # Lever 2: the producer already block-quantized to fp8, so
                # policy_weights arrive as (name, fp8) + (name + "_scale_inv",
                # scale) -- exactly the list fp8.load_weights would build. Load
                # it directly, skipping the redundant post-receipt requant.
                n_scale = sum(
                    1 for k, _ in policy_weights if k.endswith("_scale_inv")
                )
                print(
                    f"[fp8_broadcast] consumer: direct fp8 load, "
                    f"{n_scale} scale tensors received",
                    flush=True,
                )
