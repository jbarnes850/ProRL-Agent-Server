...
# Refit profiling (NRL_REFIT_PROFILE=1). Attributes weight-sync wall-clock to
# producer {generate, pack, broadcast} and consumer {receive, unpack, load}
# sub-phases. Serialized attribution: each phase is bracketed by a device
# synchronize, so overlap is removed and the per-phase device time is honest;
# the reported total may exceed the overlapped weight_sync_s. Ratios are the
# signal. Off by default -> zero effect on the measured path.
_REFIT_PROFILE = os.getenv("NRL_REFIT_PROFILE") == "1"


def _sync() -> None:
    if _REFIT_PROFILE and torch.cuda.is_available():
...
class _PhaseTimer:
    ...
    def report(self, tag: str) -> None:
        parts = " ".join(f"{k}={v:.2f}s" for k, v in self.acc.items())
        total = sum(self.acc.values())
        print(
            f"[refit_profile] {tag}: {parts} total={total:.2f}s buckets={self.n}",
            flush=True,
        )
...
def packed_broadcast_producer(iterator, group, src, post_iter_func):
    ...
    """
    prof = _PhaseTimer() if _REFIT_PROFILE else None
        ...
        with torch.cuda.stream(streams[buffer_idx]):  # type: ignore[arg-type]
            try:
                ...
                while True:
                    ...
                    if prof is not None:
                        _sync()
                        prof.add("generate", time.perf_counter() - _t)
                ...
                if prof is not None:
                    _sync()
                    prof.add("pack", time.perf_counter() - _t)
                    _t = time.perf_counter()
                group.broadcast(packed_tensors[buffer_idx], src=src)
                if prof is not None:
                    _sync()
                    prof.add("broadcast", time.perf_counter() - _t)
                    prof.n += 1
    ...
    while True:
        ...
        with torch.cuda.stream(streams[buffer_idx]):  # type: ignore[arg-type]
            ...
            try:
                ...
                if prof is not None:
                    _sync()
                    prof.add("receive", time.perf_counter() - _t)
                    _t = time.perf_counter()
                _unpacked = unpack_tensor(
                    packed_tensors[buffer_idx], packing_tensor_meta_data[buffer_idx]
                )
                if prof is not None:
                    _sync()
                    prof.add("unpack", time.perf_counter() - _t)
                    _t = time.perf_counter()
                # Load the packed tensor into the model
                post_unpack_func(_unpacked)
                if prof is not None:
                    _sync()
                    prof.add("load", time.perf_counter() - _t)
                    prof.n += 1
            except StopIteration:
                # do the last broadcast if there are remaining tensors
