#!/usr/bin/env python3
"""Submit one count_stars task to every supported harness."""
...
from submit_count_stars_task import (
    DEFAULT_BACKEND,
    DEFAULT_NUM_SAMPLES,
    DEFAULT_TOPOLOGY,
    EXAMPLE_DIR,
...
def print_combined_summary(
    ...
    for harness in results:
        s = summaries[harness]
        rtext = ", ".join("n/a" if r is None else f"{r:.1f}" for r in s["rewards"])
        print(
    ...
    with httpx.Client(base_url=rollout_url, timeout=timeout) as client:
        ...
        while len(finished) < len(harnesses):
            ...
            if newly_done:
                ...
                for harness in newly_done:
                    done = finished[harness]["completed_sessions"]
                    total = finished[harness]["total_sessions"]
                    print(f"  [{elapsed:>5.0f}s] {harness:<16} done ({done}/{total})")
            else:
                sys.stdout.write(
